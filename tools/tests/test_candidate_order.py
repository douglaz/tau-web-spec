...
class Selection(unittest.TestCase):
    def test_historical_record(self):
        ...
        session = {"context_floor": 500000, "depth": 5, "allowlist": []}
    ...
    def test_filters_order_depth_and_makers(self):
        ...
        session = {"allowlist": ["allow", "allow-short", "allow-no-tools"],
                   "context_floor": 500000, "depth": 4}
        selected = select.select(select.model_list(payload(*models)), session)
...
class Update(unittest.TestCase):
    ...
    def test_changed_order_preserves_unrelated_values_and_treats_slugs_as_data(self):
        ...
        with patch.object(job, "request_json", return_value=(200, data)):
            self.assertTrue(job.update_bundle(self.path))
        after = select.read_bundle(self.path)
        self.assertEqual(after["session"].pop("model"),
                         [{"slug": slug, "maker": "Maker unrelated to slug"}])
        before["session"].pop("model")
        self.assertEqual(before, after)
        self.assertIn('provider = "z-ai"', self.path.read_text())
    ...
    def test_missing_secret_fails_before_any_network_call(self):
        self.path.write_text(bundle_source(allowlist=["private-choice"]))
        before = self.path.read_bytes()
    ...
    def allowlisted(self):
        chosen = {"slug": "private-choice", "maker": "Maker unrelated to slug"}
        self.path.write_text(bundle_source([chosen], [chosen["slug"]]))
    ...
    def test_probe_success_sends_case_g_routing_shape(self):
        ...
        with patch.object(job, "request_json", side_effect=[(200, data), (200, response)]) as request:
            self.assertTrue(job.update_bundle(self.path))
        ...
        self.assertEqual(select.read_bundle(self.path)["session"]["allowlist"], ["private-choice"])
    ...
    def test_refusal_outside_candidate_order_is_no_proposal(self):
        data = self.allowlisted()
        self.path.write_text(bundle_source(select.select(data["data"], {
            "allowlist": [], "context_floor": 500000, "depth": 5}), ["private-choice"]))
        before = self.path.read_bytes()
        refusal = json.loads((FINDING / "run-2026-09-23/case-C.json").read_text())
        with patch.object(job, "request_json", side_effect=[(200, data), (404, refusal)]):
            self.assertFalse(job.update_bundle(self.path))
...
class Proposal(unittest.TestCase):
    def test_real_git_proposal_create_update_and_noop(self):
        ...
        def command(*args):
            ...
            self.assertIn("Publisher review", Path(args[args.index("--body-file") + 1]).read_text())
            pr_exists = True
        ...
        with tempfile.TemporaryDirectory() as temp:
            ...
            with contextlib.chdir(work):
                ...
                with patch.object(sys, "argv", ["propose_candidates.py", "--default-branch", "main"]), \
                    ...
                    for slug in ("next", "later"):
                        real_command("git", "checkout", "main")
                        with patch.object(job, "request_json", return_value=(200, payload(model(slug)))):
                            job.main()
                        self.assertEqual(real_command("git", "branch", "--show-current"), job.BRANCH)
                        proposed = select.read_bundle(Path("bundle/inference.toml"))
