...
- an explicit empty list `[]` — the tenant has said *none*, and **what *none* means is the
  field's own**, stated in its row below. A field that declares what must exist takes *nothing
  may*, and anything found is a finding; a field that declares a restriction or an obligation
  takes *none is declared*, and there is nothing to check at delivery — a list the maintained
  re-check runs is such an obligation, and delivery reads only whether it is present. Which
  reading a field takes, and whether its list is the re-check's, is the table's last column. Until 2026-09-16 this bullet read "nothing to check, delivery may pass" for every field,
...
| Field | Value | What the check does | What the declaration decides |
|---|---|---|---|
| `version` | `1` | Refuse any other | **number** |
| `listeners.inbound` | list of `{proto: "tcp"\|"udp", port: n, from: "any"\|"peers"\|"none"}` | Every answering socket must match an entry; an undeclared one is the finding (`CNF-50`, `CNF-51`) | **nothing may** |
